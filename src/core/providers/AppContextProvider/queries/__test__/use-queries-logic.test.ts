...
describe("useInitQueries", () => {
  describe("when api is null", () => {
    it("returns null", () => {
      const result = useInitQueries(null);
  ...
  describe("when api is provided", () => {
    ...
    let queries: ReturnType<typeof useInitQueries>;

    beforeEach(() => {
      jest.clearAllMocks();
      api = makeApi();
      queries = useInitQueries(api);
    });

    it("returns a non-null queries object", () => {
    ...
    describe("albumList", () => {
      ...
      it("queryFn calls api.getAlbumList with alphabeticalByArtist type", async () => {
      ...
      it("queryKey reflects different sizes and offsets", () => {
    ...
    describe("streamUrl", () => {
      ...
      it("queryFn calls api.buildUrl with stream method and track id", async () => {
    ...
    describe("coverArtImage", () => {
      ...
      it("is enabled when entityId is provided", () => {
        const options = queries!.coverArtImage("album-1", 256);
        expect(options.enabled).toBe(true);
      }, 30000);

      it("queryFn returns cached artwork when cache hit", async () => {
      ...
      it("queryFn fetches from API when cache miss", async () => {
      ...
      it("queryFn uses size * 2 for the API request", async () => {
        (Image.getCachePathAsync as jest.Mock).mockResolvedValue(null);
      ...
      it("has staleTime set to undefined", () => {
        const options = queries!.coverArtImage("entity-1", 256);
    ...
    describe("song", () => {
      ...
      it("queryFn calls api.getSong with correct id", async () => {
    ...
    describe("search", () => {
      ...
      it("queryFn calls api.search2 with correct params", async () => {
      ...
      it("filters albums by title matching query (case insensitive)", async () => {
        (api.search2 as jest.Mock).mockResolvedValue({
          searchResult2: {
            album: [
      ...
      it("filters songs by title matching query (case insensitive)", async () => {
        (api.search2 as jest.Mock).mockResolvedValue({
          searchResult2: {
            album: [],
            artist: [],
      ...
      it("returns artists unfiltered", async () => {
        const artists = [
      ...
      it("handles undefined album and song gracefully", async () => {
        (api.search2 as jest.Mock).mockResolvedValue({
          searchResult2: {
            album: undefined,
            artist: [],
    ...
    describe("album", () => {
      ...
      it("queryFn calls api.getAlbum with correct id", async () => {
    ...
    describe("artists", () => {
      ...
      it("queryFn calls api.getArtists", async () => {
    ...
    describe("artist", () => {
      ...
      it("queryFn calls api.getArtist with correct id", async () => {
