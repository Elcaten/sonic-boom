...
describe("batchProcess", () => {
  ...
  it("handles rejected promises without throwing", async () => {
  ...
  it("passes accumulated results to onProgress", async () => {
  ...
  it("processes a single batch without delay when only one batch exists", async () => {
    const promises = [Promise.resolve("a"), Promise.resolve("b")];
    const results = await batchProcess({ promises, pageSize: 10, delayMs: 100 });
    expect(results).toHaveLength(2);
  }, 30000);

  it("applies delay between batches when delayMs is set", async () => {
...
describe("batchProcessWithDetails", () => {
  it("returns correct counts for all fulfilled promises", async () => {
    const promises = [Promise.resolve(1), Promise.resolve(2), Promise.resolve(3)];
  ...
  it("returns correct counts for mixed fulfilled and rejected promises", async () => {
    const error = new Error("oops");
    const promises = [Promise.resolve("ok"), Promise.reject(error), Promise.resolve("also ok")];
    const details = await batchProcessWithDetails({ promises });
  ...
  it("returns correct counts when all promises reject", async () => {
  ...
  it("includes all settled results in the results field", async () => {
    const error = new Error("fail");
